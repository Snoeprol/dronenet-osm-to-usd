...
def create_textured_cube():
    # Create the stage
    stage = Usd.Stage.CreateNew("output/basic_textured_cube.usda")
    ...
    texCoords = UsdGeom.PrimvarsAPI(cube).CreatePrimvar(
        ...
        UsdGeom.Tokens.faceVarying  # Changed to faceVarying for per-face texturing
    )
    ...
    # Set the texture file path (using absolute path for better reliability)
    diffuseTexture.CreateInput("file", Sdf.ValueTypeNames.Asset).Set("./texture.jpg")
